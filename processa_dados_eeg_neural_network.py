...
def get_results(arquivo, amostra):
    ...
    for i in range(1,janelas+1):
        ...
        y = y['final']
        #y=butterworth(y,Wn=0.4, tipo='low')
        y=butterworth(y,Wn=0.25, tipo='high', N=8)
...
#        plt.show()
        ...
        output = get_output_array(amostra)
...
for index, amostra in enumerate(usaveis):
    print(amostra)
    kap, output=get_results(filenames[amostra], amostra)
    print(output)
    if index == 0:
        final_dataset = kap
        final_output = output
